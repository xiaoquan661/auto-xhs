...
description: |
  ...
  Profile、连接身份校验、安全换号、并发调度、认证登录、内容发布、搜索发现、
...
- 发布、评论等不可逆操作不得根据上下文猜测账号，必须向用户明确展示目标账号并确认。
...
# 2. 启动该账号的 Bridge 和独立 Chrome；首次未配对时会报告未就绪
python scripts/cli.py --account brand-a account-start

# 3. 新账号或首次升级旧账号：同步通用扩展，在目标 Profile 弹窗中粘贴一次性配对包
