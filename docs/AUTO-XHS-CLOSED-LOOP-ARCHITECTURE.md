...
主动私信批次只由 Agent/Python CLI 下发，WebUI 只读展示任务及逐人结果。收到的新私信走独立的
WebUI 收件箱流程：只读采集、AI 草稿、逐条人工确认、发送回读，不开放无人值守自动回复。
...
| 5 WebUI | AUTOMATED_TESTED_PARTIAL | 六区工作台、任务与结果、发布单任务确认、主动私信逐人结果、评论/私信收件箱、AI 草稿和模型配置 | 指标历史可视化、私信确认发送与完整真实浏览器流程验收 |
