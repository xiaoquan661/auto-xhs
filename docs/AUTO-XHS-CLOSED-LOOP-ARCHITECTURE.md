...
| 回复       | 发现自己笔记下的新评论，生成被动任务并回复         | 单次回复动作已有；监测、规则和被动任务未实现 |
| 数据一直做 | 持续回收任务执行记录和运营指标                     | 最小执行记录已有；持续运营数据回收未实现     |
...
| 2 被动回复 | PARTIAL | 自动发现自己最近笔记、评论读取、事件收件箱、唯一被动任务、草稿确认、结果闭环、回复规则底座 | WebUI 收件箱、规则自动执行、真实账号验收 |
| 3 对外能力 | PARTIAL | 主加已完成页面执行器并真实验收；私信已完成首次/续发适配器、1–10 人个性化批次、整批确认、逐人回读和 Agent/CLI 执行链 | 私信真实发送验收、主页装修和两类拉群执行链 |
| 4 运营数据 | PARTIAL | 账号与笔记指标采集、SQLite 时间快照、历史和相邻快照增量 API | 曝光/阅读数据源、日报周报、调度策略、真实账号验收 |
| 5 WebUI | API_PARTIAL | 控制面板已有任务与结果展示，私信批次可只读查看逐人状态；入站事件、操作记录、回复规则、指标历史/增量、动作草稿 API | 收件箱和数据中心界面；不为后续能力默认增加任务入口 |
| 6 真实验收 | PARTIAL | 主加已用测试账号完成两次真实关注并回读“已关注” | 其余新增能力逐项验收 |

主加已通过专项自动化测试，并用测试账号完成两个随机目标的真实关注和结果回读。私信执行链已通过
自动化测试；本轮只完成真实页面的只读结构检查，没有发送真实私信。
