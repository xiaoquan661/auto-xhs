...
| `generate-reply-draft` | 为一个明确的新评论事件生成待人工确认草稿 |
| `collect-operations-metrics` | 保存账号和自己笔记的运营指标时间快照 |

一步发布命令不属于 V1.5 产品流程。资料修改和后台规则化自动回复尚无完整命令时不得猜测；
`generate-reply-draft` 只生成草稿，不代表已经发送回复。
...
禁止从研究步骤直接调用一步发布命令。发布必须进入分步填写、真实浏览器预览和逐任务确认链。
...
需要 AI 草稿时，对一个明确事件运行 `generate-reply-draft`。该命令使用评论上下文、可选账号人格和
知识文件生成待人工确认文本；模型未配置或返回无效结构时不创建草稿，也不会退回固定模板发送。
