...
- 发布预览仍由 Agent/Python CLI 创建；进入 `preview_ready` 后，用户可在 WebUI 核对账号、UID、内容、素材和发布时间，并确认发布或保存草稿。
- V1.5 必须采用“填写 → 浏览器预览 → 用户确认 → 点击发布”，禁止使用一步直发。
- 私信和公开资料修改虽然属于 V1.5 目标，但不属于本技能；执行链完成前明确报告待实现。

## 🔒 技能边界（强制）

**所有发布操作只能通过本项目的 CLI 或 WebUI 受控确认服务完成，不得使用任何外部项目的工具：**

- **唯一执行实现**：Agent 使用 `python scripts/cli.py <子命令>`；WebUI 只允许确认已有 `preview_ready` 任务，并复用本项目发布适配器和任务状态机。
...
- **WebUI 边界**：WebUI 不创建或填写发布任务，不支持批量确认；每个任务必须单独核对真实浏览器预览，账号与当前 UID 匹配后才能确认。
