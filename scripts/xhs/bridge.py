...
class BridgePage:
    ...
    def is_server_running(self) -> bool:
        ...
        return self.get_server_status() is not None

    def get_server_status(self) -> dict | None:
        """Return Bridge and connected extension identity metadata."""
        try:
    ...
    def is_extension_connected(self) -> bool:
        ...
        return bool(status and status.get("extension_connected"))

    def clear_extension_binding(self) -> bool:
        """Ask the connected universal extension to erase this Profile binding."""
        try:
            result = self._call("unpair_extension") or {}
            return bool(result.get("binding_cleared"))
        except Exception:
            return False
    ...
    def shutdown_server(
        ...
        *,
        ...
        """Ask the matching local Bridge process to stop cleanly."""
        try:
            with ws_client.connect(self._bridge_url, open_timeout=3) as ws:
                ws.send(json.dumps({
                    "role": "cli",
                ...
                raw = ws.recv(timeout=5)
            return bool(json.loads(raw).get("result", {}).get("shutting_down"))
        except Exception:
            return False
