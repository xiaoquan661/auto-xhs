...
importScripts("bridge_config.js", "netlogger.js");
netlogInit().catch(e => console.warn("[XHS NetLogger] init failed", e));

const BOOTSTRAP_CONFIG = globalThis.XHS_BRIDGE_CONFIG || {
  mode: "universal",
  storageKey: "xhsBridgeBinding",
};
const BINDING_STORAGE_KEY = BOOTSTRAP_CONFIG.storageKey || "xhsBridgeBinding";
const PAIRING_PREFIX = "xhs-pair-v1:";
let binding = null;
let ws = null;
...
async function loadBinding() {
  ...
  const stored = await chrome.storage.local.get(BINDING_STORAGE_KEY);
  binding = stored[BINDING_STORAGE_KEY] || null;

  // One-time migration for old per-account extension copies.
  if (!binding && BOOTSTRAP_CONFIG.account && BOOTSTRAP_CONFIG.account !== "default" &&
      BOOTSTRAP_CONFIG.accountId && BOOTSTRAP_CONFIG.bridgeToken) {
    binding = {
      schemaVersion: 1,
      account: BOOTSTRAP_CONFIG.account,
      bridgeUrl: BOOTSTRAP_CONFIG.bridgeUrl,
      accountId: BOOTSTRAP_CONFIG.accountId,
      bridgeToken: BOOTSTRAP_CONFIG.bridgeToken,
      profileDirectory: BOOTSTRAP_CONFIG.profileDirectory || "Default",
      instanceId: await getExtensionInstanceId(),
    };
    await chrome.storage.local.set({ [BINDING_STORAGE_KEY]: binding });
  }
  return binding;
}
...
function publicStatus() {
  return {
    paired: !!binding,
    wsConnected: handshakeVerified,
    account: binding?.account || null,
    bridgeUrl: binding?.bridgeUrl || null,
...
chrome.alarms.onAlarm.addListener(() => {
  if (!ws || ws.readyState !== WebSocket.OPEN) loadBinding()
  .then(() => connect())
  .catch(e => console.error("[XHS Bridge] 初始化失败", e));
});
...
function broadcastStatus() {
  chrome.runtime.sendMessage({ type: "STATUS_CHANGED", status: publicStatus() }).catch(() => {});
}

chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  if (msg.type === "GET_STATUS") {
    loadBinding()
      .then(() => sendResponse({ success: true, status: publicStatus() }))
      .catch(e => sendResponse({ success: false, error: String(e.message || e) }));
    return true;
  }

  if (msg.type === "PAIR_EXTENSION") {
    pairExtension(String(msg.bundle || ""))
      .then(status => sendResponse({ success: true, status }))
      .catch(e => sendResponse({ success: false, error: String(e.message || e) }));
    return true;
  }

  if (msg.type === "CLEAR_LOCAL_PAIRING") {
    clearLocalBinding()
      .then(() => sendResponse({ success: true, status: publicStatus() }))
      .catch(e => sendResponse({ success: false, error: String(e.message || e) }));
    return true;
...
async function getExtensionInstanceId() {
  ...
  const key = "xhsExtensionInstanceId";
  const stored = await chrome.storage.local.get(key);
  extensionInstanceId = stored[key] || crypto.randomUUID();
  if (!stored[key]) await chrome.storage.local.set({ [key]: extensionInstanceId });
  return extensionInstanceId;
}

async function connect() {
  const current = await loadBinding();
  if (!current) {
    setStatus(false);
    broadcastStatus();
    return;
  ...
  const socket = new WebSocket(current.bridgeUrl);
  ws = socket;
  ...
  socket.onopen = async () => {
    ...
    socket.send(JSON.stringify({
      ...
      bridge_token: current.bridgeToken,
      instance_id: instanceId,
      extension_id: chrome.runtime.id,
      profile_directory: current.profileDirectory || "Default",
    }));
  };
  ...
  socket.onmessage = async (event) => {
    ...
    if (msg.error && !msg.id) {
      console.error("[XHS Bridge] 身份校验失败:", msg.error);
      setStatus(false);
      broadcastStatus();
      socket.close();
      return;
...
async function clearLocalBinding() {
  binding = null;
  handshakeVerified = false;
  await chrome.storage.local.remove(BINDING_STORAGE_KEY);
  if (ws) {
    const active = ws;
    ws = null;
    active.close();
...
function decodePairingBundle(bundleText) {
  if (!bundleText.startsWith(PAIRING_PREFIX)) throw new Error("配对包格式无效");
  const encoded = bundleText.slice(PAIRING_PREFIX.length).trim();
  ...
  try {
    ...
    throw new Error("配对包无法解析");
  }
  if (payload?.v !== 1 || !/^[A-Za-z0-9][A-Za-z0-9_-]{0,31}$/.test(payload.account || "")) {
    throw new Error("配对包版本或账号格式无效");
  }
  if (!/^ws:\/\/(localhost|127\.0\.0\.1):\d{4,5}$/.test(payload.bridgeUrl || "")) {
    throw new Error("配对 Bridge 必须是本机 WebSocket 地址");
  }
  if (!payload.pairingCode || (payload.expiresAt && Date.parse(payload.expiresAt) <= Date.now())) {
    throw new Error("配对包已过期或缺少配对码");
  }
  return payload;
}

async function pairExtension(bundleText) {
  const payload = decodePairingBundle(bundleText);
  const instanceId = await getExtensionInstanceId();
  const result = await new Promise((resolve, reject) => {
    const pairingWs = new WebSocket(payload.bridgeUrl);
    const timer = setTimeout(() => {
      pairingWs.close();
      reject(new Error("配对请求超时"));
    }, 15000);
    pairingWs.onopen = () => pairingWs.send(JSON.stringify({
      role: "pairing",
      account: payload.account,
      pairing_code: payload.pairingCode,
      instance_id: instanceId,
      extension_id: chrome.runtime.id,
      profile_directory: payload.profileDirectory || "Default",
    }));
    pairingWs.onmessage = event => {
      let message;
      try { message = JSON.parse(event.data); } catch { return; }
      if (message.type === "pairing_ack" && message.binding) {
        clearTimeout(timer);
        pairingWs.close();
        resolve(message.binding);
      } else if (message.type === "pairing_error" || message.error) {
        clearTimeout(timer);
        pairingWs.close();
        reject(new Error(message.error || "配对失败"));
      }
    };
    pairingWs.onerror = () => {
      clearTimeout(timer);
      reject(new Error("无法连接配对 Bridge"));
    };
  });

  if (result.instanceId !== instanceId || result.account !== payload.account) {
    throw new Error("Bridge 返回的配对身份与当前扩展不一致");
  }
  if (ws) {
    const active = ws;
    ws = null;
...
async function handleCommand(msg) {
  ...
  switch (method) {
    case "unpair_extension":
      setTimeout(() => clearLocalBinding().catch(() => {}), 250);
      return { binding_cleared: true };
